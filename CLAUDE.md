...
| 2026-08-10 | Web ve e-posta altyapısı **stüdyo domaini** `stforge.dev` altında toplandı; site ayrı bir repoda (`SamedTemiz/stforge-site`, private), Cloudflare Pages/Workers üzerinde | Domain uygulamaya değil stüdyoya ait: her uygulama bir klasör (`/nisteia`, sonra `/app2`). İkinci uygulamanın sayfasını Nisteia reposuna commit etmek zorunda kalmamak için site uygulama reposundan çıkarıldı. E-posta: Cloudflare Email Routing ile sınırsız alias, tek gmail kutusuna düşüyor — uygulama başına mailbox yok |
| 2026-08-10 | Puan isteği Play'in In-App Review API'siyle, **önüne memnuniyet sorusu konmadan** | "Uygulamayı beğendiniz mi? → evet ise mağazaya yönlendir" deseni yaygın ama Google bunu açıkça yasaklıyor (inceleme akışını filtrelemek politika ihlali, uygulama kaldırılabilir). Eşikler `lib/features/review/review_prompt.dart` içinde saf Dart ve testli. API kotayı gizlediği ve sayfayı gösterip göstermediğini söylemediği için "soruldu" işareti iyimser konuyor; 120 gün sonra bir sigorta denemesi daha yapılıp `kMaxAsks`'te duruluyor — süre kontrolü tek başına bırakılırsa sonsuza kadar 120 günde bir sorar. Sayaç yalnızca onboarding bittikten sonra işlemeye başlar: kurulumu yarım bırakılan günler "kullanım" değil |
| 2026-08-10 | Mağaza açıklamasından ve IAP ürün açıklamasından teslim edilmemiş Pro vaatleri (widget, öğün planlayıcı, tarifler) kaldırıldı | Uygulama içi paywall 2026-07-14'ten beri dürüsttü ama mağaza metni kayıtsız şartsız vaat etmeye devam ediyordu — kullanıcı widget bekleyerek satın alıyordu. Yayınlanmamış özellik "yakında" diye de yazılmıyor (Google politikası + beklenti yaratıyor) |
